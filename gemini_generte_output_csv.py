...
INPUT_JSON = "/mnt/d/Naved/Outputs/jdcr_derm_vignettes/2022-2025/gemini-3-pro-preview-image-r3.json"
OUTPUT_XLSX = "/mnt/d/Naved/Outputs/jdcr_derm_vignettes/2022-2025/xlsx/gemini-3-pro-preview-image-r3.xlsx"
...
for issue_key, questions_dict in data.items():
    issue = normalize_issue(issue_key)

    if not isinstance(questions_dict, dict):
        continue

    for qnum, qdata in questions_dict.items():
        if not qnum.startswith("Q"):
            continue

        answer = qdata.get("answer_letter", "")

        # Normalize invalid answers
        if answer == "[INVALID]" or answer is None:
            answer = ""

        lookup[(issue, qnum)] = answer
...
print(f"Total rows (excluding header): {len(ISSUE_ORDER) * 3}")
